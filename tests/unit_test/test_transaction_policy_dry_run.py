...
def test_transaction_run_when_policy_dry_run_is_false():
    t = ValidateLocal
    t.execution_policy = TransactionPolicy(dry_run=False)
...
def test_transaction_doesnt_run_when_policy_dry_run_is_true(caplog):
    t = ValidateLocal
    t.execution_policy = TransactionPolicy(dry_run=True)
    Application().at(t)
