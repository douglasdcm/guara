...
from tests.unit_test.test_unit_app_examples.async_todo_list.operations import (
    Add,
    ListTasks,
    Remove,
)
...
class TestAsyncToDo:
    ...
    async def test_async_add_task(self, setup_test: Coroutine[Any, Any, None]) -> None:
        ...
        await (
            self._todo.at(transaction=Add, task=task)
            .asserts(IsEqualTo, expected)
            .perform()
        )
    ...
    async def test_async_remove_task(
        ...
        await (
            self._todo.at(transaction=Remove, task=task)
            .asserts(IsEqualTo, expected)
            .perform()
        )
