...
class FlakyTransaction(AbstractTransaction):
    execution_policy = TransactionExecutionPolicy(return_on_dry_run=Error1())
...
def test_application_retry_when_exception_in_list(caplog):
    ...
    app = Application(ApplicationExecutionPolicy(retry_on_exceptions=(Error1)))
    with raises(Error1):
...
def test_application_not_retry_when_exception_not_in_list(caplog):
    caplog.set_level(logging.INFO)
    app = Application(ApplicationExecutionPolicy(retry_on_exceptions=(Error2, Error3)))
    with raises(Error1):
