...
def get_retries_on_failure() -> int:
    try:
        ...
        if result > 0:
            LOGGER.warning(f"RETRIES_ON_FAILURE: {result}. Transactions will be retried on failure.")
            return result
        ...
        LOGGER.error(f"Invalid RETRIES_ON_FAILURE value: '{os.getenv('RETRIES_ON_FAILURE')}'. Expected an integer. Defaulting to 0.")
        return 0
