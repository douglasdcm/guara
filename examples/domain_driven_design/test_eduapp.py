...
# Tests
# =========================
@pytest.mark.skip("Not run in pipeline")
def test_create_student(main_fixture):
    ...
    (
        app.when(
            RunCLI,
            with_args=["--action", "create_student", "--name", "John"],
...
def test_create_course(main_fixture):
    ...
    (
        app.when(
            RunCLI,
            with_args=["--action", "create_course", "--name", "Math"],
            main_func=main_fixture,
        ).asserts(it.IsTrue)
    )
...
@pytest.mark.skip("Do not run in pipeline")
...
def test_gpa(main_fixture):
    ...
    (
        app.when(
            RunCLI, with_args=["--action", "gpa", "--student", "S1"], main_func=main_fixture
        ).asserts(it.IsTrue)
    )
